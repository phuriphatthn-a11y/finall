...
type BallType = "none" | "basketball" | "football" | "volleyball" | "human" | "cartoon";
interface Position {
  x: number;
  y: number;
}
...
const Animation: React.FC = () => {
  ...
  const [ballType, setBallType] = useState<BallType>("none");
  const [position, setPosition] = useState<Position>({ x: 0, y: 0 });

  const goRight = useRef<boolean>(true);
  const goDown = useRef<boolean>(true);
  ...
  const getBallStyle = (): React.CSSProperties => {
    ...
    const imageMap: Record<Exclude<BallType, "none">, string> = {
      basketball: "bass.jpg",
      football: "ball.jpg",
      volleyball: "voll.jpg",
      human: "human.jpg",
    ...
    const imageFile = imageMap[ballType as Exclude<BallType, "none">];
  ...
  const ballOptions: { label: string; value: BallType }[] = [
